...
static void f1()
{
    ...
    std::unique_ptr<trustwave::credentials> creds1
        = std::make_unique<trustwave::credentials>("WORKGROUP", "administrator", "finjan123", "SSS");
    for(int i = 0; i < 10; ++i) {
...
int main(int argc, char** argv)
{
    ...
    for(int i = 0; i < 20; i = i + 3) {
        sessions->get_session_by<shared_mem_sessions_cache::remote>(std::string("192.168.140.32")
                                                                    + std::to_string(i % 10));
        sleep(2);
    }
}
