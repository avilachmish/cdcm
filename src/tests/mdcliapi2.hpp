...
public:
    ...
    void connect_to_broker()
    {
        ...
        if(m_verbose) trustwave::zmq_helpers::console("I: connecting to broker at %s...", m_broker.c_str());
    }
    ...
    zmsg* recv()
    {
        ...
        zmq::pollitem_t items[] = {{static_cast<void*>(*m_client), 0, ZMQ_POLLIN, 0}};
    ...
    zmsg* send_and_recv(const std::string& tosend)
    {
        auto request1 = std::make_unique<zmsg>(tosend.c_str());
        send("echo", request1.get());
        return recv();
    }
