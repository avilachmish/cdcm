...
static void fc(int i)
{
    ...
    if (i % 2 == 0) {
        creds1 = std::make_unique<trustwave::credentials>("WORKGROUP", "administrator", "finjan123", "SSS");
        s = std::make_unique<trustwave::session>("192.168.140.32", *creds1);
    }
    else {
        creds1 = std::make_unique<trustwave::credentials>("WORKGROUP", "administrator", "finjan", "SSS");
        s = std::make_unique<trustwave::session>("192.168.140.64", *creds1);
    }
    if (!rc.connect(*s)) {
