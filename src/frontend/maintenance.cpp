...
maintenance::maintenance(boost::asio::io_service& ios):
    timer_(ios, authenticated_scan_server::instance().settings()->maintenance_interval_)
{
...
void maintenance::maintenance_function()
{
    ...
    for(const auto& entry: boost::filesystem::directory_iterator(
            authenticated_scan_server::instance().settings()->downloaded_files_path_)) {
        if(session_list.end()
           == std::find(session_list.begin(), session_list.end(),
                        std::string(entry.path().filename().string().c_str(), uuid_str_len))) {
